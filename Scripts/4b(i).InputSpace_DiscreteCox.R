...
### NOTE: Experiments conducted in sections 2 & 3 of this script informed this specification of the baseline hazard
...
### OVERALL CONCLUSION: Splines prove to be the best, but the log-transformed time
###                     model is selected for simplicity and its superior performance compared to
###                     the raw time model.
...
###       such that each spell will have its own baseline and slope
...
### RESULTS: Best Harrell's C-statistics: [g0_Delinq_SD_4]; [g0_Delinq_SD_5]; [g0_Delinq_SD_6]
...
# Discriminatory power (in-sample)
concTable(datCredit_train, datCredit_valid, vars, TimeDef=c("Cox_Discrete","DefSpell_Event"), genPath=genObjPath, modelType="Cox_Discrete")
### RESULTS: Best C-statistics: [g0_Delinq_Any_Aggr_Prop]; [g0_Delinq_Any_Aggr_Prop_Lag_12]; [g0_Delinq_Any_Aggr_Prop_Lag_1]
###          All statistics are significant

### CONCLUSION: Select top 3 variables:
###               [g0_Delinq_Any_Aggr_Prop]; [g0_Delinq_Any_Aggr_Prop_Lag_12]; [g0_Delinq_Any_Aggr_Prop_Lag_1]
...
# Discriminatory power (in-sample)
concTable(datCredit_train, datCredit_valid, vars, TimeDef=c("Cox_Discrete","DefSpell_Event"), genPath=genObjPath, modelType="Cox_Discrete")
### RESULTS: This decreases from the highest lag to lowest lag in decreasing order and the differences are minuscule
...
# Goodness-of-fit
aicTable(datCredit_train, vars, TimeDef=c("Cox_Discrete","DefSpell_Event"), genPath=genObjPath, modelType="Cox_Discrete")
### RESULTS: Best AIC-results: [g0_Delinq_Ave]; [ArrearsToBalance_1_Aggr_Prop]; [CuringEvents_Aggr_Prop]
# Discriminatory power (in-sample)
concTable(datCredit_train, datCredit_valid, vars, TimeDef=c("Cox_Discrete","DefSpell_Event"), genPath=genObjPath, modelType="Cox_Discrete")
### RESULTS: Best C-statistics: [g0_Delinq_Ave]; [ArrearsToBalance_1_Aggr_Prop]; [CuringEvents_Aggr_Prop]
###          All statistics are significant
...
# Discriminatory power (in-sample)
concTable(datCredit_train, datCredit_valid, vars, TimeDef=c("Cox_Discrete","DefSpell_Event"), genPath=genObjPath, modelType="Cox_Discrete")
### RESULTS: Best C-statistics: [Arrears]; [slc_past_due_amt_imputed_med]; [TimeInDelinqState]; [slc_curing_ind]; [slc_acct_arr_dir_3]; [slc_acct_arr_dir_3_Change_Ind]

### CONCLUSION: Select top 5 variables (preference given to Concordance statistics):
...
# --- 4.6 Fit model on combined input space
...
vars <- c("log(TimeInDefSpell)*DefSpell_Num_binned",
          ...
          "g0_Delinq_Ave", "ArrearsToBalance_1_Aggr_Prop", "CuringEvents_Aggr_Prop",
          "Arrears", "slc_past_due_amt_imputed_med", "slc_curing_ind", "slc_acct_arr_dir_3", "slc_acct_arr_dir_3_Change_Ind"
          )
...
modLR <- glm(as.formula(paste("DefSpell_Event ~", paste(vars, collapse=" + "))),
             data=datCredit_train, family="binomial")
evalLR(modLR, modLR_base, datCredit_train, targetFld="DefSpell_Event", predClass=1)
### RESULTS: AIC: 95 981; McFadden R^2: 56.74%; AUC: 98.61%
### NOTE: Quasi-complete separation occurred in fitting the full model, the
###       automated selection procedure should filter out the relevant variables contributing to this
...
###             [Arrears]; [slc_curing_ind]; [slc_acct_arr_dir_3];
###             [slc_past_due_amt_imputed_med]; 
###             [g0_Delinq_Ave]; [ArrearsToBalance_1_Aggr_Prop]; [CuringEvents_Aggr_Prop]

### CONCLUSION: Proceed with the varaibles from the automated variable selection procedure
...
### RESULTS: AIC: 213 546; McFadden R^2: 3.71%; AUC: 68.38%
...
summary(modLR_step)
evalLR(modLR_step, modLR_base, datCredit_train, targetFld="DefSpell_Event", predClass=1)
### RESULTS: AIC: 213 546; McFadden R^2: 3.71%; AUC: 68.37%
###          Variables from step-wise selection procedure:
###             [log(TimeInDefSpell)]; [DefSpell_Num_binned]; [log(TimeInDefSpell):DefSpell_Num_binned]
###             [InstalmentToBalance_1_Aggr_Prop]; [NewLoans_Aggr_Prop]; [AgeToTerm_Aggr_Mean];
###             [InterestRate_Margin_Aggr_Med_2]; [InterestRate_Margin_Aggr_Med_2]
...
# ------ 8. Combine variables from all themes

# --- 8.1 Run an automated selection procedure on the entire set
...
vars <- c("log(TimeInDefSpell)*DefSpell_Num_binned",
          ...
          "M_DTI_Growth_12", "M_DTI_Growth_6",
          "M_RealIncome_Growth_3", "M_RealIncome_Growth_6", "M_RealIncome_Growth_9",
          "M_Repo_Rate", "M_Repo_Rate_12", "M_RealGDP_Growth_3",
          "M_Inflation_Growth_1", "M_Inflation_Growth_12", "M_Emp_Growth_9")
...
modLR_step <- stepAIC(modLR_base2, scope=list(lower=~log(TimeInDefSpell)*DefSpell_Num_binned, 
                                              upper=as.formula(paste("~", paste(vars, collapse=" + ")))), 
                      direction = "both", k=log(datCredit_train[,.N]), maxit=50)
proc.time() - ptm # IGNORE: elapsed runtime; 229 minutes

# - Evaluate model
summary(modLR_step)
evalLR(modLR_step, modLR_base, datCredit_train, targetFld="DefSpell_Event", predClass=1)
### RESULTS: AIC: 49 838; McFadden R^2: 77.55%; AUC: 99.66%
###          Variables from step-wise selection procedure:
###             [log(TimeInDefSpell)]; [DefSpell_Num_binned]; [log(TimeInDefSpell):DefSpell_Num_binned];
