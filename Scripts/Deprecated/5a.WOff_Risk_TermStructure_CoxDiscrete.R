...
# --- 1.2 Create a dataset for classic write-off risk modelling | Grain = One observation per default spell
# - Create subset of performing spells towards modelling each spell as a single record | classic write-off risk modelling
datTrain_classic <- subset(datCredit_train, DefSpell_Counter==1)
datValid_classic <- subset(datCredit_valid, DefSpell_Counter==1)

# - Assign target/response field for the classic write-off risk modelling
...
# - Create a copy of [DefSpell_Age] to serve as an input into the classic LGD-model
...
modLR_Adv <- readRDS(paste0(genObjPath,"CoxDisc_Basic_Model.rds"))
...
(datSurv_sub <- surv_summary(km_Default))

# - Enrich data
datSurv_sub <- datSurv_sub %>%
  # Rename some fields
...
# ------ 3. Constructing expected term-structures of write-off | Non-dichotomised models
...
# --- 3.2 Calculate survival quantities of interest | Discrete-time models
...
datCredit[, Hazard_bas := predict(modLR_Adv, newdata=datCredit, type = "response")]
...
# --- 3.3 Calculate survival quantities of interest | Classic model
...
# ------ 4. Constructing expected term-structures of write-off | Dichotomised models
...
KM_WOff_bas <- survfit(Surv(time=TimeInDefSpell-1, time2=TimeInDefSpell, event=Youden_bas==1, type="counting") ~ 1, 
                       id=DefSpell_Key, data=datCredit)
KM_WOff_classic <- survfit(Surv(time=TimeInDefSpell-1, time2=TimeInDefSpell, event=Youden_classic==1, type="counting") ~ 1, 
                           id=DefSpell_Key, data=datCredit)


# --- 4.2 Estimate survival quantities
# - Advanced dichotomised model
# Create a survival table
(datSurv_sub_adv <- surv_summary(KM_WOff_adv))
datSurv_sub_adv <- datSurv_sub_adv %>%
  # Rename fields
  rename(Time=time, AtRisk_n=`n.risk`, Event_n=`n.event`, Censored_n=`n.censor`, SurvivalProb_KM=`surv`) %>%
  # Estiamte hazard
  mutate(Hazard_Actual=Event_n/AtRisk_n) %>%
  ...
  filter(Event_n > 0 | Censored_n >0) %>% as.data.table()
# Order data
setorder(datSurv_sub_adv, Time)
# Estiamte the at-risk proportion
datSurv_sub_adv[,AtRisk_perc := AtRisk_n/max(AtRisk_n, na.rm=T)]

# - Basic dichotomised model
(datSurv_sub_bas <- surv_summary(KM_WOff_bas)) # Survival table
datSurv_sub_bas <- datSurv_sub_bas %>%
  # Rename fields
  rename(Time=time, AtRisk_n=`n.risk`, Event_n=`n.event`, Censored_n=`n.censor`, SurvivalProb_KM=`surv`) %>%
  # Estiamte hazard
  mutate(Hazard_Actual = Event_n/AtRisk_n) %>%
  ...
  filter(Event_n > 0 | Censored_n >0) %>% as.data.table()
# Order data
setorder(datSurv_sub_bas, Time)
# Estiamte the at-risk proportion
datSurv_sub_bas[,AtRisk_perc := AtRisk_n / max(AtRisk_n, na.rm=T)]

# - Classical dichotomised model
(datSurv_sub_classic <- surv_summary(KM_WOff_classic)) # Survival table
datSurv_sub_classic <- datSurv_sub_classic %>%
  # Rename fields
  rename(Time=time, AtRisk_n=`n.risk`, Event_n=`n.event`, Censored_n=`n.censor`, SurvivalProb_KM=`surv`) %>%
  # Estiamte hazard
  mutate(Hazard_Actual = Event_n/AtRisk_n) %>% 
...
# - Add data from the dichotomised basic model
datSurv_exp <- datSurv_exp %>%
  left_join(
    datSurv_sub_bas %>% dplyr::select(Time, EventRate),   
    by = c("TimeInDefSpell" = "Time")           
  ) %>%
  rename(EventRate_bas_Youden = EventRate)

# - Add data from the dichotomised advanced model
datSurv_exp <- datSurv_exp %>%
  left_join(
    datSurv_sub_adv %>% dplyr::select(Time, EventRate),   
    by = c("TimeInDefSpell" = "Time")           
  ) %>%
  rename(EventRate_adv_Youden = EventRate)

# - Add data from the dichotomised classical model
datSurv_exp <- datSurv_exp %>%
  left_join(
