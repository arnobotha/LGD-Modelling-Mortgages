...
# --- 5.2 Investigate specific thresholds' impact on the expected term-structure
# just for inspection purposes
# - Assign cut-offs based on optimisation results
cost_DtH_bas <- 35; cost_DtH_adv <- 1; cost_LR <- 25
...
plot(datSurv_act[Time<=120, Hazard_Actual], type="b")
...
lines(datSurv_act[Time<=120, Hazard_Actual], type="b")
...
# Save graph object
dpi <- 220
ggsave(plot=g.full, filename=paste0(genFigPath, "CostMultiple_Optimisation_MAE_Hazard.png"), 
